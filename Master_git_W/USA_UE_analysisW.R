...
start <- Sys.time()
posterior_a <- sample_posterior(initial = theta0, hyperpar = hyperpar0, S = 11, S0 = 1, S_rho = 1000, S0_rho = 100, Y = Y, W = W)
end <- Sys.time()
print(end - start)
save.image(paste0("~/Desktop/Magisterka/Master_git/Master_git_W/post_simul/posterior_USA_UE_W", format(Sys.time(), "%b%d"), ".RData"))

posterior_a$convergence
########### PRIORS for illustration

load("~/Desktop/Magisterka/Master_git/Master_git_W/post_simul/posterior_USA_UE_Wmaj29.rdata")
