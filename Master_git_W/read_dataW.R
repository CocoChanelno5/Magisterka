setwd("~/Desktop/Magisterka/Master_git/Master_git_W")
...
library(spdep)
W <- matrix(0, nrow = 73, ncol = 73)
names<-PL_regions
id_list<-read_excel("~/Desktop/Magisterka/Master_git/dane/ALL_USA_PL.xlsx",
                    sheet="PL_regiony")

W <- matrix(0, nrow = 48, ncol = 48)
names<-USA_states
id_list<-read_excel("~/Desktop/Magisterka/Master_git/dane/ALL_USA_PL.xlsx",
                    sheet="USA_regiony")
i=0
...
for(ii in names) {
  ...
  for(jj in names) {
    j=j+1
    W[i, j] <- ifelse(i == j, 0, ifelse(id_list[which(id_list$Name==ii),5] == id_list[which(id_list$Name==jj),5], 1, 0))
    #print(W[i, j])#if (id_list[which(id_list$Name==ii),5] == id_list[which(id_list$Name==jj),5]) W[i, j] <- 1
    #if (i == j) W[i, j] <- 0
  }
}
...
W_USA <- W
colnames(W_USA) <- names

W_PL <- W
colnames(W_PL) <- names

'''
W_USA<- as.matrix(read_excel("~/Desktop/Magisterka/Master_git/dane/ALL_USA_PL.xlsx",
                             sheet="W_USA")[,2:49])
library(spdep)
W_USA <- mat2listw(W_USA, style = "W")
W_USA <- listw2mat(W_USA)
W==W_USA
'''
