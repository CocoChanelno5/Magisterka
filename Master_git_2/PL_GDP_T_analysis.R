...
N <- ncol(Y)
map<-PL_map
map@data$names<-unique(PL_GDP_ch$Name)
...
for (i in 1:pages){
  ...
  for (pp in 1:m) {
    ...
    if (pp<=N){ 
      ...
      axis(1, seq(1,17,1), cex.axis=cex/1.5, srt = 45,tck=-0.015, #col.axis="red",
           labels=seq(2002,2018,1))
    }}
  dev.off()
}
...
#draw_impulse_empty(PL_map,48,names,theta_posterior_means,W,e,r,1)
