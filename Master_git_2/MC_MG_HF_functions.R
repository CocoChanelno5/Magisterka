...
Hamilton_filter <- function(Y, Yl, theta, W) {
  ...
  for (tt in 1:TT) {
    for (nn in 1:N) {
      ...
      density_0[tt,nn] <- dnorm(Y[tt,nn],
                                mean = theta$rho * t(as.matrix(W[nn,])) %*% as.matrix(Y[tt,]) + theta$mu_0[nn] + theta$phi*as.matrix(Yl[tt,nn]),
                                sd = sqrt(theta$omega_d[nn]))
      density_1[tt,nn] <- dnorm(Y[tt,nn],
                                mean = theta$rho * t(as.matrix(W[nn,])) %*% as.matrix(Y[tt,]) + theta$mu_1[nn] + theta$phi*as.matrix(Yl[tt,nn]),
                                sd = sqrt(theta$omega_d[nn]))
...
loglik_uc <- function(theta, Y, Yl, W, R) {
  
  p_Hamilton <- Hamilton_filter(Y, Yl, theta, W)
  S_simul <- multiMoveGibbs(theta, p_Hamilton, R)
  loglik_cS <- lapply(S_simul, FUN = loglik_conditional_S, Y = Y, Yl = Yl, theta = theta, W = W)
...
relist <- function(theta_vector, N) {
  theta <- list(rho = as.vector(theta_vector[1]),
                phi = as.vector(theta_vector[2]),
                mu_1 = as.vector(theta_vector[3:(N+2)]),
                mu_0 = as.vector(theta_vector[(N+3):(2*N+2)]),
                omega_d = as.vector(theta_vector[(2*N+3):(3*N+2)]),
                p_00 = as.vector(theta_vector[(3*N+3):(4*N+2)]),
                p_11 = as.vector(theta_vector[(4*N+3):(5*N+2)]))
  return(theta)
}
