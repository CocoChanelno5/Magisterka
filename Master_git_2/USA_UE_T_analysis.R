...
main_colour <- "navy"
main_colour2<- "deeppink3"
main_colour_phi<- "orchid3"
main_colour_rho<- "turquoise3"
...
for (i in 1:pages){
  ...
  for (pp in 1:m) {
    ...
    if (pp<=N){ 
      ...
      axis(1, seq(1,253,12), cex.axis=cex/1.5, srt = 45,tck=-0.015, #col.axis="red",
...
write.table(rbind(rho.sum,phi.sum), file = "rho&phi_results_t.csv", sep = ";", dec = ",")
...
draw_impulse2<-function(map,N,n,theta,W,ef,r,legend,i){
  ...
  pal[1:6] <- brewer.pal(6, "PuBuGn")
  pal<-rev(pal)
  pal[7:10] <- brewer.pal(5, "Oranges")[2:5]
  impulse2 <- as.matrix(rep(0,N))
